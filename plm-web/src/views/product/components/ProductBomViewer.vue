    ...
    <template v-else>
      ...
      <section v-else class="viewer-section">
        <div v-if="summary?.formalVersions.length" class="viewer-controls">
          ...
          <el-select v-model="selectedRouteId" aria-label="工艺路线"><el-option v-for="route in selectedVersion?.routes || []" :key="route.productBomRouteId" :label="route.routeName" :value="route.productBomRouteId" /></el-select>
        </div>
        <template v-if="selectedRoute">
          <div class="route-summary"><div><strong>{{ selectedRoute.routeName }}</strong><code>{{ selectedRoute.routeCode }}</code></div><div class="route-colors"><el-tag v-for="color in selectedRoute.colors" :key="color" size="small">{{ color }}</el-tag></div><strong>{{ selectedRoute.costSnapshot ? `${selectedRoute.costSnapshot.currencyCode} ${selectedRoute.costSnapshot.totalCost}` : '成本计算失败或尚未计算' }}</strong></div>
