...
浏览器
  ...
  -> 远程 PostgreSQL：<DB_HOST>:5432/plm_yuewei?currentSchema=plm
  -> Docker 内部 Redis
```
...
```env
DB_URL=jdbc:postgresql://<DB_HOST>:5432/plm_yuewei?currentSchema=plm
...
docker run --rm -e PGPASSWORD=<DB_PASSWORD> postgres:15-alpine \
  psql -h <DB_HOST> -p 5432 -U postgres -d plm_yuewei \
...
```bash
pg_restore -h <DB_HOST> -p 5432 -U postgres -d plm_yuewei --clean --if-exists 备份文件.dump
```
